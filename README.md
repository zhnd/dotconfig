...
| `dot_config/nvim/` | `~/.config/nvim/` | LazyVim 配置（init.lua + lua/ + lazy-lock.json 等） |
...
### LazyVim plugin 更新后同步 lockfile

```sh
nvim                              # 在里面 :Lazy update
chezmoi add ~/.config/nvim/lazy-lock.json
chezmoi cd && git commit -am "chore: bump nvim plugins" && git push
```
